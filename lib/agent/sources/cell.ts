...
// Cell Host & Microbe — přes PubMed API (spolehlivější než scraping cell.com)
...
export async function fetchCell(): Promise<RawStudy[]> {
  try {
    // Search PubMed for Cell Host & Microbe journal
    const searchRes = await axios.get(`${PUBMED_BASE}/esearch.fcgi`, {
      params: {
        db: "pubmed",
        term: 'microbiome AND "Cell Host Microbe"[Journal]',
        reldate: 1,
        datetype: "pdat",
        retmax: 30,
        retmode: "json",
